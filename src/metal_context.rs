...
use objc2_metal::{
    ...
    MTLCreateSystemDefaultDevice, MTLDevice, MTLLibrary, MTLSize,
};
use std::ffi::c_void;
use std::ptr::NonNull;
...
impl GeMMMetalContext {
    pub fn gemm_init() -> Self {
        ...
        let lib = match (device
            // .newLibraryWithFile_error(ns_string!("../shader/naive.metallib"))
            // .or_else(|_err| {
            .newLibraryWithSource_options_error(
                ns_string!(include_str!("../shaders/block_tiling.metal")),
                None,
            )) {
            Ok(res) => res,
            Err(err) => {
        ...
        let func = lib
            .newFunctionWithName(ns_string!("gemm_tensor_kernel"))
            .unwrap();
        let state = device
    ...
    pub fn bind_data(&mut self, a: &mut Matrix<f32>, b: &mut Matrix<f32>) {
        ...
        let thread_group_nb = MTLSize {
            width: (n as usize).div_ceil(TILE_SIZE),
            height: (m as usize).div_ceil(TILE_SIZE),
            depth: 1,
        };

        unsafe {
            self.compute_encoder
                .setBuffer_offset_atIndex(self.a_buffer.as_deref(), 0, 0);
            self.compute_encoder
                .setBuffer_offset_atIndex(self.b_buffer.as_deref(), 0, 1);
            self.compute_encoder
                .setBuffer_offset_atIndex(self.c_buffer.as_deref(), 0, 2);
            self.compute_encoder
                .setBuffer_offset_atIndex(sizes_uniform_buffer.as_deref(), 0, 3);
            self.compute_encoder
                .dispatchThreadgroups_threadsPerThreadgroup(thread_group_nb, thread_group_size);
        }
    }
    ...
    pub unsafe fn compute(&mut self) -> &[f32] {
        ...
        unsafe {
            std::slice::from_raw_parts(result, c_buffer.length() / std::mem::size_of::<f32>())
        }
    }
}
